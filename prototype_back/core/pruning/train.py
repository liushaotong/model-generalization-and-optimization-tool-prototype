...
import torch
import os
from core.pruning.utils import display_structure, loss_fn_kd, loss_label_smoothing, display_factor, display_structure_hyper, \
    LabelSmoothingLoss
import copy
import torch.nn.functional as F
from torch.utils.data import DataLoader
from core.pruning.models.sampler import ImbalancedAccuracySampler
...
def train_epm(train_loader, model, optimizer, optimizer_p, epoch, epm_flag, nf, resource_constraint, hyper_net=None,
    ...
    if len(epm) > inner_bn:
        ...
        epm_flag = (True and epm_flag)



    else:
        epm_flag = (False and epm_flag)
    ...
    for batch_idx, (inputs, targets) in enumerate(tqdm_loader):
        ...
        if epm_flag:  # 如果使用记忆模块的损失
            # epm_vector = vector
            vector = vector + nf * torch.randn(vector.size()).cuda()
...
def valid(epoch, net, testloader, best_acc, best_state, hyper_net=None, stage='valid_model', flag=False):
    txtdir = './txt/'
    ...
    if flag:
        if is_best:
            ...
            # torch.save(best_state, './checkpoint/%s.pth.tar' % (model_string))

    print('Loss: %.3f | Acc: %.3f%% (%d/%d) | Best Acc: %.3f%%'
          % (test_loss / len(testloader), 100. * correct / total, correct, total, best_acc))

    return best_acc, test_loss / len(testloader), 100. * correct / total, best_state
